...
The three colored paths on the supply page (corridor holds, standoff, corridor lapses) are conditional model projections. Each is the EIA baseline from the September 9 outlook, adjusted month by month. The adjustments are analyst judgment — a documented choice, not a reported figure. The full assumptions live in the dated snapshot `research/sources/world-stocks-assumptions-v2026-09-17.json` (the version each forecast record references by sha256; the working copy is `site/src/data/world-stocks-assumptions.json`), and every number on the page is computed from it, not re-typed — the checked-in scripts that do the computing and drift-checking are described at the end of this section.
