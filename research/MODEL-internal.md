...
2. **Three branches, weights reweighted Sep 11** (10/40/50; before that 10/50/40 since Sep 9, 15/55/30 Sep 7):
   ...
   - ***Sep 15 evening — judgment UNCHANGED (10/40/50, user decision):*** filter re-run (prior 10/40/50, transits 4/d, 1 tanker loss this week, Brent settled **108.50 — new closing record**, corridor flow absent) → **horizon 5.5/11.8/82.7** (was 5.6/12.6/81.8). The record close is the Yanbu buffer being priced (Rystad/CNBC: the ~4M b/d pipeline loss vs the 5–7 day cover; "prices are unlikely to fall below $100 anytime soon"), not a regime event — no clause fired. Elasticity per +$5 from $101.39: −0.25/−0.51/−0.76 mb/d. The Yanbu clock stays the standing trigger candidate; the Sep 16 WPSR (w/e Sep 11) + Fed decision remain the day's gates.
3. **SPR draw rate is the leading indicator, not the level.** 285.4M (w/e Sep 4, released Sep 10 — ingested late Sep 15 after the watchlist carried the wrong week-ending); the w/e Sep 4 draw was just **1.24M bbl (~0.18M b/d), down about 60% from the ~0.45M b/d pace** carried since the program nominally ended. The continuation after the IEA program is unexplained and is the most policy-dependent line in the model — one DOE announcement flattens the curve. **Check: the Sep 16 print (w/e Sep 11) — a second slow week (or a DOE statement) is the reweight surface.**
